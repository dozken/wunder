...
> Mirrored from https://wundernn.io/connectome/docs/submission_guide (fetched 2026-09-11).
...
Data overview.
Remember to handle the model's internal state. When you encounter a new sequence (a new seq_ix), you must reset the state.
...
CPU1 vCPU
RAM16 GB
Inference time60 minutes for the entire test set
Submission size20 MB for the uploaded ZIP
GPUNot available; CPU inference only
Internet accessDisabled
...
The Rules are also worth reading.
